...
def plot_model_history(model_history, l):
    ...
    plt.title('Learning Curve for ' + str(l) + " percent variance")
    plt.xlabel('Iter #')
...
for i, l in enumerate(pca_list):
    ...
    num_inp = pca.n_components_
    
    x_train = pca.transform(x_train)
    x_test = pca.transform(x_test)

    print("running for " +str(l) + "percent variance")
    t1 = time.time()
...
pca_arr = np.array(pca_list).reshape(len(len_vec),1)
...
results = np.concatenate((h1_arr, act_arr, opt_arr, lr_arr, epoch_arr, pca_arr, mse_arr, acc_arr, time_arr), axis=1)    

result = pd.DataFrame(results)
result.to_csv("pca_exp.csv")

error_plots = pd.DataFrame(loss_plots)
error_plots.to_csv("error_plots_pca.csv")
