...
# Learning curves for number of PEs in last layer

h1_topo = pd.read_csv("pe_error_plots.csv")
h1_topo = np.array(h1_topo.iloc[:,1:6])

h1_topo_exp = pd.read_csv("pe_exp.csv")
h1_topo_exp = np.array(h1_topo_exp.iloc[:,[1,6,7,8]])
...
for i in range(h1_topo.shape[1]):
    ...
    plt.grid(True)
    plt.title('Learning curves for various number of PEs in last hidden layer')
    plt.xlabel("Epoch #")
    plt.ylabel("Function value")
    plt.xlim((0,9))
               
plt.legend(leg)
plt.savefig("pe_LC.png")
...
plt.title("MSE Vs Number of PEs in last hidden layer")
plt.xlabel("Number of PEs")
plt.ylabel("MSE (percent)")
plt.grid(True)
plt.savefig("pe_mse.png")

plt.figure()
#plt.subplot(3,1,2)
plt.plot(h1_topo_exp[:,i], h1_topo_exp[:,i+2])
plt.title("Accuracy (percent) Vs Number of PEs in last hidden layer")
plt.xlabel("Number of PEs")
...
plt.savefig("pe_acc.png")

#################################################


# Learning curves for split results

h2_topo = pd.read_csv("split_error_plots.csv")
h2_topo = np.array(h2_topo.iloc[:,1:6])

h2_topo_exp = pd.read_csv("split_exp.csv")
h2_topo_exp = np.array(h2_topo_exp.iloc[:,[1,6,7,8]])

i = 0

leg = [str(int(round(i*70000))) for i in h2_topo_exp[:,i]]
...
for i in range(h2_topo.shape[1]):
    ...
    plt.grid(True)
    plt.title('Learning curves for different training data sizes')
    plt.xlabel("Epoch #")
    plt.ylabel("Function value")
    plt.xlim((0,9))
               
plt.legend(leg)
plt.savefig("split_LC.png")
...
plt.savefig("split_mse.png")

plt.figure()
#plt.subplot(3,1,2)
plt.plot(h2_topo_exp[:,i], h2_topo_exp[:,i+2])
plt.title("Accuracy (percent) Vs training size")
plt.xlabel("Number of training samples")
plt.ylabel("Accuracy (percent)")
plt.grid(True)
...
lr_sgd = pd.read_csv("lr_error_plots.csv")
lr_sgd = np.array(lr_sgd.iloc[:,1:6])

lr_sgd_exp = pd.read_csv("lr_exp.csv")
lr_sgd_exp = np.array(lr_sgd_exp.iloc[:,3:8])
...
data = pd.read_csv("epochs_error_plots.csv")
data = np.array(data.iloc[:,1:6])
...
for i in range(data.shape[1]):
    ...
    plt.grid(True)
    plt.title('Learning curves for different epoch numbers')
    plt.xlabel("Epoch #")
    plt.ylabel("Function value")
    plt.xlim((0,49))
