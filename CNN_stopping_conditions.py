...
num_epochs = [10, 20, 30, 40, 50] #Number of proceesing elements
...
len_vec = num_epochs #this variable is to save time - no need to change variable names
#in results part.... 

loss_plots = np.zeros((max(num_epochs), (len(len_vec))))
...
for i, l in enumerate(num_epochs):
    
    print("running for "+ str(l) + " epochs")
    t1 = time.time()
    ...
    model_info = model.fit(x_train_reshaped, y_train, validation_split= 0.1, epochs=l, verbose=2)
...
split = [3/4]* len(len_vec)
sp_arr = np.array(split).reshape(len(len_vec),1)
...
opt = ['Adam'] * len(len_vec)
opt_arr = np.array(opt).reshape(len(len_vec),1)

#Number of epcohs
#epoc = [10] * len(len_vec)
epoch_arr = np.array(num_epochs).reshape(len(len_vec),1)
...
results = np.concatenate((sp_arr, act_arr, opt_arr, lr_arr, epoch_arr, mse_arr, acc_arr, time_arr), axis=1)    

result = pd.DataFrame(results)
result.to_csv("epochs_exp.csv")

#Saving values for learning curves
error_plots = pd.DataFrame(loss_plots)
error_plots.to_csv("epochs_error_plots.csv")
